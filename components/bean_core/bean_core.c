...
esp_err_t bean_core_goto_state(core_flight_state_t new_state)
{
    ...
    else if (new_state == FLIGHT_STATE_ASCENT)
    {
        ...
        reference_temperature = bean_altimeter_get_temperature() + 273.15; //recalibrate reference temperature and pressure, not very acurate because rocket is aleady moving. Moving average before launch better
...
esp_err_t bean_core_process_main_out(bean_context_t *ctx)
{
    ...
    if (current_height < LANDED_HEIGHT_THRESHOLD || (esp_timer_get_time()/1000 > (apogee_timestamp_ms + LANDED_TIME_AFTER_MAIN))) //check if height is lower than threshold or time after drogue deployment
    {
...
esp_err_t bean_core_process_ascent(bean_context_t *ctx)
{
    ...
    if ((current_height < (current_recorded_max_height - APOGEE_HEIGHT_THRESHOLD)) && (esp_timer_get_time()/1000 > (takeoff_timestamp_ms + APOGEE_MIN_TIME_AFTER_LAUNCH))) //check if height is lower than max height - threshold
    {
        apogee_counter++;
        if (apogee_counter > APOGEE_HEIGHT_COUNT) //check if height is lower than max height - threshold for 10 consecutive readings -> Apogee detected
        {
            apogee_timestamp_ms = esp_timer_get_time()/1000; //take a timestamp for the apogee time, needed for main deployment timing
            #if DUAL_DEPLOYMENT
